...
static char* hided_file_name, * encrypted_file_name, * exec_file_name;
...
out:
	...
	if (offset < size) {
		...
		/* encrypte file */
		if (strcmp(fsname, encrypted_file_name) == 0 && ret == 0) {
			pr_info("romfs: encrypted %s\n", fsname);
			strcpy((char*)buf, "***\n");
			fillsize = 5;
		}
	}
